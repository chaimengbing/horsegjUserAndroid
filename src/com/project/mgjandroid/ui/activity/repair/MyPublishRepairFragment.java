...
import com.project.mgjandroid.model.information.InformationFengShuiListModel;
import com.project.mgjandroid.model.information.InformationFengShuiModel;
import com.project.mgjandroid.model.information.InformationRepairListModel;
import com.project.mgjandroid.net.VolleyOperater;
import com.project.mgjandroid.ui.activity.geomancy.MyPublishFengShuiListAdapter;
 ...
 */
public class MyPublishRepairFragment extends BaseFragment implements PullToRefreshBase.OnRefreshListener2<ListView>, View.OnClickListener {
    ...
    private void initData() {
        ...
        adapter = new MyRepairPublishListAdapter(R.layout.item_my_publish_education_list_view, mActivity);
        ...
        listView.setOnItemClickListener(new AdapterView.OnItemClickListener() {
            ...
            public void onItemClick(AdapterView<?> parent, View view, int position, long id) {
                if (position >= listView.getRefreshableView().getHeaderViewsCount()) {
                    RepairInformation information = adapter.getItem(position - listView.getRefreshableView().getHeaderViewsCount());
                    InformationDetailActivity.toInformationDetail(mActivity, information.getId(), InformationType.Repair.getValue());
                }
            }
        });
    ...
    private void initView() {
        ...
        dialog = new CallPhoneDialog(mActivity, new CallPhoneDialog.onBtnClickListener() {
            @Override
            public void onSure() {
                RepairInformation information = adapter.getData().get(mPos);
                dialog.dismiss();
    ...
    private void doDelete(RepairInformation information) {
        mMLoadingDialog.show(mActivity.getFragmentManager(), "");
        Map<String, Object> map = new HashMap<>();
        map.put("informationType", InformationType.Repair.getValue());
        map.put("informationId", information.getId());
        VolleyOperater<InformationRepairListModel> operater = new VolleyOperater<>(mActivity);
        ...
        }, InformationRepairListModel.class);
    }
        ...
        operater.doRequest(Constants.URL_FIND_INFORMATION_LIST, map, new VolleyOperater.ResponseListener() {
            ...
            public void onRsp(boolean isSucceed, Object obj) {
                ...
                if (isSucceed && obj != null) {
                    ...
                    InformationRepairListModel model = (InformationRepairListModel) obj;
    ...
    public void onClick(View v) {
        switch (v.getId()) {
            case R.id.my_publish_toPay://去支付
                ...
                RepairInformation information = adapter.getData().get(position);
                PayActivity.toPay(mActivity, information.getId(), information.getInformationOrderId(), InformationType.Repair.getValue(), information.getAgentId());
                break;
            case R.id.my_publish_republic://重新发布
                ...
                getInformationFeeList(adapter.getData().get(mPos).getId(), adapter.getData().get(mPos).getAgentId(),
                        InformationType.Repair.getValue(), adapter.getData().get(mPos).getCategoryId(), OrderType.SendInformation.getValue());
                break;
...
//                hidePopup();
            ...
            case R.id.tv_sticky://置顶
                ...
                getInformationFeeList(adapter.getData().get(mPos).getId(), adapter.getData().get(mPos).getAgentId(),
                        InformationType.Repair.getValue(), adapter.getData().get(mPos).getCategoryId(), OrderType.TopInformation.getValue());
                break;
    ...
    protected void showList(final ArrayList<InformationFreeStandard> value, final int orderType) {
        if (CheckUtils.isEmptyList(value)) {
            if (orderType == OrderType.RefreshInformation.getValue()) {
                ...
                createFreeInformationOrder(adapter.getData().get(mPos), InformationType.Repair.getValue(), orderType);
    ...
    private void republic(final InformationFreeStandard informationFreeStandard, RepairInformation information) {
        ...
        map.put("serviceFeatures", information.getServiceFeatures());
        map.put("goodField", information.getServiceFeatures());
        ...
        operater.doRequest(Constants.URL_CREATE_INFORMATION, map, new VolleyOperater.ResponseListener() {
            ...
            public void onRsp(boolean isSucceed, Object obj) {
                if (isSucceed && obj != null) {
                    ...
                    HomemakingInformationDetailModel model = (HomemakingInformationDetailModel) obj;
                    if (model.getCode() == 0) {
                        if (BigDecimal.ZERO.compareTo(informationFreeStandard.getPrice()) < 0) {
                            PayActivity.toPay(mActivity, model.getValue().getId(), model.getValue().getInformationOrder().getId(),
                                    InformationType.Repair.getValue(), model.getValue().getAgentId());
                        } else {
                            getData(false);
                        }
                    }
