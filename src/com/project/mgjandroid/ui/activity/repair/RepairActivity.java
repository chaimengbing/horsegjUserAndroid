...
import com.project.mgjandroid.model.RepairModel;
...
@Router("repair")
public class RepairActivity extends BaseActivity implements PullToRefreshBase.OnRefreshListener2, AdapterView.OnItemClickListener {
    public static final String IS_FROM_REPAIR_ACTIVITY = "repair_activity";
    ...
    @InjectView(R.id.repair_list_view)
    private PullToRefreshListView mListView;
    @InjectView(R.id.repair_container)
...
//        type = ChooseCityModel.getInstance().getSecondHandType();
        ...
        mAdapter = new RepairListAdapter(R.layout.item_repair_list_view, mActivity);
...
//        mListView.setEmptyView(emptyView);
        ...
        switch (v.getId()) {
            ...
            case R.id.tv_to_publish:
                if (CommonUtils.checkLogin(mActivity)) {
                    Intent publish = new Intent(mActivity, PublishRepairInfoActivity.class);
    ...
    public void getTag() {
        ...
        operater.doRequest(Constants.URL_REPAIR_HOT_CATEGORY_LIST, null, new VolleyOperater.ResponseListener() {
    ...
    private void getData() {
        ...
        if (categoryId != -1) map.put("repairCategoryId", categoryId);
        if (ChooseCityModel.getInstance().getCityCode() != 0)
            map.put("cityCode", ChooseCityModel.getInstance().getCityCode());
        if (ChooseCityModel.getInstance().getProvinceId() != 0)
            map.put("province", ChooseCityModel.getInstance().getProvinceId());
        ...
        if (ChooseCityModel.getInstance().getDistrictId() != 0)
            map.put("district", ChooseCityModel.getInstance().getDistrictId());
        VolleyOperater<RepairModel> operater = new VolleyOperater<>(mActivity);
        operater.doRequest(Constants.URL_REPAIR_LIST, map, new VolleyOperater.ResponseListener() {
            @Override
            public void onRsp(boolean isSucceed, Object obj) {
                ...
                if (isSucceed && obj != null) {
                    ...
                    RepairModel recruitListModel = (RepairModel) obj;
                    ...
                    if (CheckUtils.isNoEmptyList(mlist)) {
                        ...
                        if (START != 0) {
                            ...
                            List<RepairInformation> mlistOrg = mAdapter.getData();
                            if (mlistOrg != null) {
...
//                        ChooseCityModel.getInstance().setType(type);
                ...
                textView.setOnClickListener(new View.OnClickListener() {
                    @Override
                    public void onClick(View v) {
                        Intent allTag = new Intent(mActivity, RepairCategoryActivity.class);
                        allTag.putExtra("from", RepairActivity.IS_FROM_REPAIR_ACTIVITY);
