...
import com.project.mgjandroid.ui.activity.information.ChoosePublishIdentityActivity;
...
public class PublishRecyclingInformationActivity extends BaseActivity {

    private static final int GET_AREA = 103;
    private static final int GET_IDENTITY = 104;
    private static final int GET_SERVICE_CLASS = 105;
    ...
    protected void onCreate(Bundle arg0) {
        super.onCreate(arg0);
        setContentView(R.layout.activity_publish_recycling_information);
    ...
    private void initView() {
        ...
        tvServiceClass.setOnClickListener(this);
        tvReleaseArea.setOnClickListener(this);
        tvGetCode.setOnClickListener(this);
        tvChange.setOnClickListener(this);
        tvPublish.setOnClickListener(this);
        etDetailedInformation.addTextChangedListener(new TextWatcher() {
            @Override
    ...
    public void onClick(View v) {
        switch (v.getId()) {
            ...
            case R.id.tv_identity:
                CommonUtils.hideKeyBoard(mActivity);
                ChoosePublishIdentityActivity.toChoosePublishIdentity(mActivity, GET_IDENTITY);
                break;
            case R.id.tv_service_class:
                CommonUtils.hideKeyBoard(mActivity);
                InformationCategoryChooseActivity.toChooseInformationCategory(mActivity, InformationType.WasteRecovery.getValue(), InformationCategoryChooseActivity.FROM_INFORMATION_PUBLISH_PAGE, GET_SERVICE_CLASS);
                break;
...
//                    tvPublish.setEnabled(false);
                    getInformationFeeList(null, agentId, InformationType.WasteRecovery.getValue(), mPositionType, OrderType.SendInformation.getValue());
                }
                break;
            case R.id.tv_change:
                changeGetCodeLabel("");
    ...
    protected void onActivityResult(int requestCode, int resultCode, Intent data) {
        if (resultCode == RESULT_OK && data != null) {
            switch (requestCode) {
                case GET_IDENTITY:
                    String identity = data.getStringExtra(ChoosePublishIdentityActivity.AGENT);
                    identityCount = data.getIntExtra("identityCount", -1);
                    tvIdentity.setText(identity);
                    break;
                case GET_SERVICE_CLASS:
    ...
    private void publish(final InformationFreeStandard informationFreeStandard) {
        ...
        map.put("informationType", InformationType.WasteRecovery.getValue());
        map.put("imgs", imageUrls.toString());
        map.put("mobile", etPhone.getText().toString().trim());
        map.put("description", etDetailedInformation.getText().toString().trim());
        map.put("type", identityCount);
        map.put("serviceArea", etServiceArea.getText().toString().trim());
        map.put("goodField", tvServiceClass.getText());
                        ...
                        if (informationFreeStandard == null) {
                            ...
                            if (BigDecimal.ZERO.compareTo(informationFreeStandard.getPrice()) < 0) {
                                PayActivity.toPay(mActivity, model.getValue().getId(), model.getValue().getInformationOrder().getId(),
                                        InformationType.WasteRecovery.getValue(), model.getValue().getAgentId());
                            } else {
//                                InformationDetailActivity.toInformationDetail(mActivity, model.getValue().getId(), InformationType.Recruit.getValue());
                                MyPublishInformationActivity.toMyPublishInformationList(mActivity, InformationType.WasteRecovery.getValue());
                            }
                        }
                        finish();
                    }
    ...
    private boolean checkCanPublish() {
        if (TextUtils.isEmpty(etTitle.getText().toString().trim())) {
            ToastUtils.displayMsg("请填写标题内容", mActivity);
            return false;
        }
        if (etTitle.getText().toString().trim().length() < 6 || etTitle.getText().toString().trim().length() > 30) {
            ToastUtils.displayMsg("标题字数限制为最少6个字，最多30个字", mActivity);
            return false;
        }
        if (TextUtils.isEmpty(tvIdentity.getText().toString().trim())) {
            ToastUtils.displayMsg("请选择您的身份", mActivity);
            return false;
        }
        if (TextUtils.isEmpty(tvServiceClass.getText().toString().trim())) {
            ToastUtils.displayMsg("请选择所属的类别", mActivity);
            return false;
        }
        if (TextUtils.isEmpty(tvServiceClass.getText().toString().trim())) {
            ToastUtils.displayMsg("请选择您所服务的类别", mActivity);
            return false;
        }
        if (TextUtils.isEmpty(etClass.getText().toString().trim())) {
            ToastUtils.displayMsg("请填写您所服务的区域", mActivity);
            return false;
        }
        if (etServiceArea.getText().toString().trim().length() < 2 || etServiceArea.getText().toString().trim().length() > 16) {
            ToastUtils.displayMsg("服务区域字数限制为最少2个字，最多16个字", mActivity);
            return false;
        }
        ...
        if (TextUtils.isEmpty(etDetailedInformation.getText().toString().trim())) {
            ToastUtils.displayMsg("请填写详细信息", mActivity);
            return false;
        }
        if (etDetailedInformation.getText().toString().length() < 50) {
