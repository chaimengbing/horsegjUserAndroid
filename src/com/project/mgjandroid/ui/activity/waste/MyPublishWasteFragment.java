...
import com.project.mgjandroid.ui.activity.employment.MyPublishRecruitListAdapter;
 ...
 */
public class MyPublishWasteFragment extends BaseFragment implements PullToRefreshBase.OnRefreshListener2<ListView>, View.OnClickListener {

    @InjectView(R.id.list_view)
    private PullToRefreshListView listView;
    private MyPublishWasteListAdapter adapter;
    private int start = 0;
    ...
    private void initData() {
        ...
        adapter = new MyPublishWasteListAdapter(R.layout.item_my_publish_waste, mActivity);
        ...
        listView.setOnItemClickListener(new AdapterView.OnItemClickListener() {
            ...
            public void onItemClick(AdapterView<?> parent, View view, int position, long id) {
                if (position >= listView.getRefreshableView().getHeaderViewsCount()) {
                    InformationWasteRecovery information = adapter.getItem(position - listView.getRefreshableView().getHeaderViewsCount());
                    InformationDetailActivity.toInformationDetail(mActivity, information.getId(), InformationType.WasteRecovery.getValue());
                }
            }
        });
    ...
    private void initView() {
        ...
        dialog = new CallPhoneDialog(mActivity, new CallPhoneDialog.onBtnClickListener() {
            @Override
            public void onSure() {
                InformationWasteRecovery information = adapter.getData().get(mPos);
                dialog.dismiss();
    ...
    private void doDelete(InformationWasteRecovery information) {
        mMLoadingDialog.show(mActivity.getFragmentManager(), "");
        Map<String, Object> map = new HashMap<>();
        map.put("informationType", InformationType.WasteRecovery.getValue());
        map.put("informationId", information.getId());
        VolleyOperater<InformationWasteListModel> operater = new VolleyOperater<>(mActivity);
        ...
        }, InformationWasteListModel.class);
    }
        ...
        operater.doRequest(Constants.URL_FIND_INFORMATION_LIST, map, new VolleyOperater.ResponseListener() {
            ...
            public void onRsp(boolean isSucceed, Object obj) {
                ...
                if (isSucceed && obj != null) {
                    ...
                    InformationWasteListModel model = (InformationWasteListModel) obj;
                    List<InformationWasteRecovery> mlist = new ArrayList<>();
                    ...
                    if (CheckUtils.isNoEmptyList(mlist)) {
                        if (isLoadMore) {
                            ...
                            List<InformationWasteRecovery> mlistOrg = adapter.getData();
                            if (mlistOrg != null) {
    ...
    public void findAgent(int position) {
        mMLoadingDialog.show(mActivity.getFragmentManager(), "");
        InformationWasteRecovery bean = adapter.getData().get(position);
        Map<String, Object> map = new HashMap<>();
            ...
            public void onRsp(boolean isSucceed, Object obj) {
                ...
                if (isSucceed && obj != null) {
                    ...
                    if (((FindAgentModel) obj).getValue() != null) {
                        ((MyPublishInformationActivity) mActivity).showStickyDialog(((FindAgentModel) obj).getValue().getPhone());
                    }
                }
            }
    ...
    public void onClick(View v) {
        switch (v.getId()) {
            case R.id.my_publish_toPay://去支付
                ...
                InformationWasteRecovery information = adapter.getData().get(position);
                PayActivity.toPay(mActivity, information.getId(), information.getInformationOrderId(), InformationType.WasteRecovery.getValue(), information.getAgentId());
                break;
            case R.id.my_publish_republic://重新发布
                ...
                getInformationFeeList(adapter.getData().get(mPos).getId(), adapter.getData().get(mPos).getAgentId(),
                        InformationType.WasteRecovery.getValue(), adapter.getData().get(mPos).getCategoryId(), OrderType.SendInformation.getValue());
                break;
...
//                hidePopup();
            ...
            case R.id.tv_sticky://置顶
                ...
                getInformationFeeList(adapter.getData().get(mPos).getId(), adapter.getData().get(mPos).getAgentId(),
                        InformationType.WasteRecovery.getValue(), adapter.getData().get(mPos).getCategoryId(), OrderType.TopInformation.getValue());
                break;
    ...
    protected void showList(final ArrayList<InformationFreeStandard> value, final int orderType) {
        if (CheckUtils.isEmptyList(value)) {
            if (orderType == OrderType.RefreshInformation.getValue()) {
                ...
                createFreeInformationOrder(adapter.getData().get(mPos), InformationType.WasteRecovery.getValue(), orderType);
    ...
    private void republic(final InformationFreeStandard informationFreeStandard, InformationWasteRecovery information) {
        ...
        map.put("serviceArea", information.getServiceArea());
