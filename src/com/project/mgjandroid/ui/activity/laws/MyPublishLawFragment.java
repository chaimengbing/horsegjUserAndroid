...
import com.project.mgjandroid.model.information.InformationLawListModel;
import com.project.mgjandroid.model.information.InformationLawModel;
import com.project.mgjandroid.model.information.InformationWasteListModel;
...
import com.project.mgjandroid.ui.activity.waste.MyPublishWasteListAdapter;
 ...
 */
public class MyPublishLawFragment extends BaseFragment implements PullToRefreshBase.OnRefreshListener2<ListView>, View.OnClickListener {

    @InjectView(R.id.list_view)
    private PullToRefreshListView listView;
    private MyPublishLawListAdapter adapter;
    private int start = 0;
    ...
    private void initData() {
        ...
        adapter = new MyPublishLawListAdapter(R.layout.item_my_publish_law, mActivity);
        ...
        listView.setOnItemClickListener(new AdapterView.OnItemClickListener() {
            ...
            public void onItemClick(AdapterView<?> parent, View view, int position, long id) {
                if (position >= listView.getRefreshableView().getHeaderViewsCount()) {
                    InformationLaw information = adapter.getItem(position - listView.getRefreshableView().getHeaderViewsCount());
                    InformationDetailActivity.toInformationDetail(mActivity, information.getId(), InformationType.Law.getValue());
                }
            }
        });
    ...
    private void initView() {
        ...
        dialog = new CallPhoneDialog(mActivity, new CallPhoneDialog.onBtnClickListener() {
            @Override
            public void onSure() {
                InformationLaw information = adapter.getData().get(mPos);
                dialog.dismiss();
    ...
    private void doDelete(InformationLaw information) {
        mMLoadingDialog.show(mActivity.getFragmentManager(), "");
        Map<String, Object> map = new HashMap<>();
        map.put("informationType", InformationType.Law.getValue());
        map.put("informationId", information.getId());
        VolleyOperater<InformationLawListModel> operater = new VolleyOperater<>(mActivity);
        ...
        operater.doRequest(Constants.URL_FIND_INFORMATION_LIST, map, new VolleyOperater.ResponseListener() {
            ...
            public void onRsp(boolean isSucceed, Object obj) {
                ...
                if (isSucceed && obj != null) {
                    ...
                    InformationLawListModel model = (InformationLawListModel) obj;
                    List<InformationLaw> mlist = new ArrayList<>();
                    ...
                    if (CheckUtils.isNoEmptyList(mlist)) {
                        if (isLoadMore) {
                            ...
                            List<InformationLaw> mlistOrg = adapter.getData();
                            if (mlistOrg != null) {
    ...
    public void onClick(View v) {
        switch (v.getId()) {
            case R.id.my_publish_toPay://去支付
                ...
                InformationLaw information = adapter.getData().get(position);
                PayActivity.toPay(mActivity, information.getId(), information.getInformationOrderId(), InformationType.Law.getValue(), information.getAgentId());
                break;
            case R.id.my_publish_republic://重新发布
                ...
                getInformationFeeList(adapter.getData().get(mPos).getId(), adapter.getData().get(mPos).getAgentId(),
                        InformationType.Law.getValue(), adapter.getData().get(mPos).getCategoryId(), OrderType.SendInformation.getValue());
                break;
...
//                hidePopup();
            ...
            case R.id.tv_sticky://置顶
                ...
                getInformationFeeList(adapter.getData().get(mPos).getId(), adapter.getData().get(mPos).getAgentId(),
                        InformationType.Law.getValue(), adapter.getData().get(mPos).getCategoryId(), OrderType.TopInformation.getValue());
                break;
            ...
            case R.id.iv_more_setting:
                mPos = (int) v.getTag();
                if (adapter.getData().get(mPos).getStatus() == 2) {
                    initPopup();
                    if (adapter.getData().get(mPos).getIsExpire() == 0 && mPos == adapter.getData().size() - 1 && adapter.getData().size() != 1) {
    ...
    protected void showList(final ArrayList<InformationFreeStandard> value, final int orderType) {
        if (CheckUtils.isEmptyList(value)) {
            if (orderType == OrderType.RefreshInformation.getValue()) {
                ...
                createFreeInformationOrder(adapter.getData().get(mPos), InformationType.Law.getValue(), orderType);
    ...
    private void republic(final InformationFreeStandard informationFreeStandard, InformationLaw information) {
        ...
        map.put("description", information.getPersonalProfile());
        map.put("headImg", information.getHeadImg());

        map.put("type", 1);
        map.put("name", information.getName());
        map.put("whereCompany", information.getWhereCompany());
        map.put("certificateOfPractice", information.getCertificateOfPractice());
        map.put("practiceTime", information.getPracticeTime());
        map.put("goodField", information.getGoodField());
        map.put("whereProvince", information.getWhereProvince());
        map.put("whereProvinceName", information.getWhereProvinceName());
        map.put("personalProfile", information.getPersonalProfile());
