...
public class ChooseCityActivity extends BaseActivity implements AreaClick {
    ...
    @InjectView(R.id.choose_city_label)
    private RelativeLayout rlChooseLabel;
    @InjectView(R.id.hot_city_grid_view)
    ...
    @InjectView(R.id.history_city_grid_view)
    private NoScrollGridView gvHistoryCity;
...
//                hotCityBean.setCityName(cityBean.getCityName());
        ...
        if (CheckUtils.isNoEmptyStr(data)) {
            final InformationAreaModel.ValueBean valueBean = JSON.parseObject(data, InformationAreaModel.ValueBean.class);
            if (CheckUtils.isNoEmptyStr(valueBean.getName())) {
                tvLocationCity.setText(valueBean.getName());
                tvLocationCity.setOnClickListener(new View.OnClickListener() {
                    @Override
                    public void onClick(View v) {
                        HotCityBean hotCityBean = new HotCityBean();
                        hotCityBean.setCityCode("");
                        hotCityBean.setCityName(valueBean.getName());
                        hotCityBean.setProvince(valueBean.getProvince());
                        hotCityBean.setCity(valueBean.getCity());
                        hotCityBean.setDistrict(valueBean.getDistrict());
                        ...
                        if (mFrom != null && mFrom.equals(JobMessageActivity.FROM_JOB_MESSAGE)) {
                            ...
                            intent.putExtra(HOT_CITY, hotCityBean);
                            ...
                            ChooseCityModel.getInstance().setProvinceId(hotCityBean.getProvince());
                            ChooseCityModel.getInstance().setCityId(hotCityBean.getCity());
                            ChooseCityModel.getInstance().setDistrictId(hotCityBean.getDistrict());
                            ChooseCityModel.getInstance().setCityName(hotCityBean.getCityName());
        ...
        if (CheckUtils.isEmptyStr(data) || CheckUtils.isEmptyStr(JSON.parseObject(data, InformationAreaModel.ValueBean.class).getName())) {
    ...
    private void getHotCity() {
        ...
        operater.doRequest(Constants.URL_FIND_HOT_CITY_LIST, map, new VolleyOperater.ResponseListener() {
