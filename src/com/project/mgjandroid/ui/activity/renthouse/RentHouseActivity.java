...
import com.project.mgjandroid.ui.adapter.RentHouseListAdapter;
...
public class RentHouseActivity extends BaseActivity implements PullToRefreshBase.OnRefreshListener2, AdapterView.OnItemClickListener {

    public static final String IS_FROM_RENT_HOUSE_ACTIVITY = "rent_house_activity";
    ...
    @InjectView(R.id.rent_house_list_view)
    private PullToRefreshListView mListView;
    @InjectView(R.id.rent_house_container)
    ...
    private void initView() {
        ...
        mAdapter = new RentHouseListAdapter(R.layout.item_rent_house_list_view, mActivity);
        ...
        if (type == 1) {
            tvEmpty.setText("暂无求租信息");
        } else if (type == 2) {
            tvEmpty.setText("暂无租房信息");
        }
//        mListView.setEmptyView(emptyView);
        ...
        switch (v.getId()) {
            ...
            case R.id.tv_to_publish:
                if (CommonUtils.checkLogin(mActivity)) {
                    Intent publish = new Intent(mActivity, PublishHouseLeaseInfoActivity.class);
    ...
    public void getTag() {
        ...
        operater.doRequest(Constants.URL_RENT_HOUSE_HOT_CATEGORY_LIST, null, new VolleyOperater.ResponseListener() {
    ...
    private void getData() {
        ...
        map.put("type", type);
        if (categoryId != -1) map.put("houseLeaseCategoryId", categoryId);
        if (ChooseCityModel.getInstance().getCityCode() != 0)
            map.put("cityCode", ChooseCityModel.getInstance().getCityCode());
        if (ChooseCityModel.getInstance().getProvinceId() != 0)
            map.put("province", ChooseCityModel.getInstance().getProvinceId());
        ...
        operater.doRequest(Constants.URL_RENT_HOUSE_LIST, map, new VolleyOperater.ResponseListener() {
            @Override
            public void onRsp(boolean isSucceed, Object obj) {
                ...
                if (isSucceed && obj != null) {
                    ...
                    if (CheckUtils.isNoEmptyList(mlist)) {
                        ...
                        if (START != 0) {
                            ...
                            List<HouseLeaseInformation> mlistOrg = mAdapter.getData();
                            if (mlistOrg != null) {
...
//                        ChooseCityModel.getInstance().setType(type);
                ...
                textView.setOnClickListener(new View.OnClickListener() {
                    @Override
                    public void onClick(View v) {
                        Intent allTag = new Intent(mActivity, HouseLeaseCategoryActivity.class);
                        allTag.putExtra("from", RentHouseActivity.IS_FROM_RENT_HOUSE_ACTIVITY);
