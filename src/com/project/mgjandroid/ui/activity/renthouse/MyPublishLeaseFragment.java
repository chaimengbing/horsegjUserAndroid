 ...
 */
public class MyPublishLeaseFragment extends BaseFragment implements PullToRefreshBase.OnRefreshListener2<ListView>, View.OnClickListener {
        ...
        switch (v.getId()) {
            ...
            case R.id.my_publish_republic://重新发布
                ...
                getInformationFeeList(adapter.getData().get(mPos).getId(), adapter.getData().get(mPos).getAgentId(),
                        InformationType.Lease.getValue(), adapter.getData().get(mPos).getCategoryId(), OrderType.SendInformation.getValue());
                break;
...
//                hidePopup();
            ...
            case R.id.tv_sticky://置顶
                ...
                getInformationFeeList(adapter.getData().get(mPos).getId(), adapter.getData().get(mPos).getAgentId(),
                        InformationType.Lease.getValue(), adapter.getData().get(mPos).getCategoryId(), OrderType.TopInformation.getValue());
                break;
            ...
            case R.id.my_publish_toPay:
                ...
                PayActivity.toPay(mActivity, information.getId(), information.getInformationOrderId(), InformationType.Lease.getValue(), information.getAgentId());
                break;
        }
    }
    ...
    private void republic(final InformationFreeStandard informationFreeStandard, HouseLeaseInformation information) {
        ...
        map.put("houseArea", information.getHouseArea());
