...
import com.project.mgjandroid.model.MyPublishSecondModel;
...
import com.project.mgjandroid.ui.adapter.MySecondPublishListAdapter;
 ...
 */
public class MyPublishSecondHandFragment extends BaseFragment implements PullToRefreshBase.OnRefreshListener2<ListView>, View.OnClickListener {

    @InjectView(R.id.list_view)
    private PullToRefreshListView listView;
    private MySecondPublishListAdapter adapter;
    private int start = 0;
    ...
    private void initData() {
        ...
        adapter = new MySecondPublishListAdapter(R.layout.item_my_publish_education_list_view, mActivity);
        ...
        listView.setOnItemClickListener(new AdapterView.OnItemClickListener() {
            ...
            public void onItemClick(AdapterView<?> parent, View view, int position, long id) {
                if (position >= listView.getRefreshableView().getHeaderViewsCount()) {
                    SecondhandInformation information = adapter.getItem(position - listView.getRefreshableView().getHeaderViewsCount());
                    InformationDetailActivity.toInformationDetail(mActivity, information.getId(), InformationType.Secondhand.getValue());
                }
            }
        });
    ...
    private void initView() {
        ...
        dialog = new CallPhoneDialog(mActivity, new CallPhoneDialog.onBtnClickListener() {
            @Override
            public void onSure() {
                SecondhandInformation information = adapter.getData().get(mPos);
                dialog.dismiss();
    ...
    private void doDelete(SecondhandInformation information) {
        mMLoadingDialog.show(mActivity.getFragmentManager(), "");
        Map<String, Object> map = new HashMap<>();
        map.put("informationType", InformationType.Secondhand.getValue());
        map.put("informationId", information.getId());
        VolleyOperater<MyPublishSecondModel> operater = new VolleyOperater<>(mActivity);
        ...
        operater.doRequest(Constants.URL_FIND_INFORMATION_LIST, map, new VolleyOperater.ResponseListener() {
            ...
            public void onRsp(boolean isSucceed, Object obj) {
                ...
                if (isSucceed && obj != null) {
                    ...
                    MyPublishSecondModel model = (MyPublishSecondModel) obj;
                    ...
                    if (CheckUtils.isNoEmptyList(mlist)) {
                        if (isLoadMore) {
                            ...
                            List<SecondhandInformation> mlistOrg = adapter.getData();
                            if (mlistOrg != null) {
    ...
    public void onClick(View v) {
        switch (v.getId()) {
            case R.id.my_publish_toPay://去支付
                ...
                SecondhandInformation information = adapter.getData().get(position);
                PayActivity.toPay(mActivity, information.getId(), information.getInformationOrderId(), InformationType.Secondhand.getValue(), information.getAgentId());
                break;
            case R.id.my_publish_republic://重新发布
                ...
                getInformationFeeList(adapter.getData().get(mPos).getId(), adapter.getData().get(mPos).getAgentId(),
                        InformationType.Secondhand.getValue(), adapter.getData().get(mPos).getCategoryId(), OrderType.SendInformation.getValue());
                break;
...
//                hidePopup();
            ...
            case R.id.tv_sticky://置顶
                ...
                getInformationFeeList(adapter.getData().get(mPos).getId(), adapter.getData().get(mPos).getAgentId(),
                        InformationType.Secondhand.getValue(), adapter.getData().get(mPos).getCategoryId(), OrderType.TopInformation.getValue());
                break;
    ...
    protected void showList(final ArrayList<InformationFreeStandard> value, final int orderType) {
        if (CheckUtils.isEmptyList(value)) {
            if (orderType == OrderType.RefreshInformation.getValue()) {
                ...
                createFreeInformationOrder(adapter.getData().get(mPos), InformationType.Secondhand.getValue(), orderType);
    ...
    private void republic(final InformationFreeStandard informationFreeStandard, SecondhandInformation information) {
        ...
        map.put("amt", information.getAmt());
        map.put("type", 2);
                    ...
                    if (model.getCode() == 0) {
                        if (BigDecimal.ZERO.compareTo(informationFreeStandard.getPrice()) < 0) {
                            PayActivity.toPay(mActivity, model.getValue().getId(), model.getValue().getInformationOrder().getId(),
                                    InformationType.Secondhand.getValue(), model.getValue().getAgentId());
                        } else {
                            getData(false);
                        }
                    }
