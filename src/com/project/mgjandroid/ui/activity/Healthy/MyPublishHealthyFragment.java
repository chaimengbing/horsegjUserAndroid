 ...
 */
public class MyPublishHealthyFragment extends BaseFragment implements PullToRefreshBase.OnRefreshListener2<ListView>, View.OnClickListener {

    @InjectView(R.id.list_view)
    private PullToRefreshListView listView;
    private MyPublishHealthyListAdapter adapter;
    private int start = 0;
    ...
    private void initData() {
        ...
        adapter = new MyPublishHealthyListAdapter(R.layout.item_my_publish_healthy, mActivity);
        ...
        listView.setOnItemClickListener(new AdapterView.OnItemClickListener() {
            ...
            public void onItemClick(AdapterView<?> parent, View view, int position, long id) {
                if (position >= listView.getRefreshableView().getHeaderViewsCount()) {
                    InformationHealth information = adapter.getItem(position - listView.getRefreshableView().getHeaderViewsCount());
                    InformationDetailActivity.toInformationDetail(mActivity, information.getId(), InformationType.Health.getValue());
                }
            }
        });
    ...
    private void initView() {
        ...
        dialog = new CallPhoneDialog(mActivity, new CallPhoneDialog.onBtnClickListener() {
            @Override
            public void onSure() {
                InformationHealth information = adapter.getData().get(mPos);
                dialog.dismiss();
    ...
    private void doDelete(InformationHealth information) {
        mMLoadingDialog.show(mActivity.getFragmentManager(), "");
        Map<String, Object> map = new HashMap<>();
        map.put("informationType", InformationType.Health.getValue());
        map.put("informationId", information.getId());
        VolleyOperater<InformationHealthListModel> operater = new VolleyOperater<>(mActivity);
        ...
        }, InformationHealthListModel.class);
    }
        ...
        operater.doRequest(Constants.URL_FIND_INFORMATION_LIST, map, new VolleyOperater.ResponseListener() {
            ...
            public void onRsp(boolean isSucceed, Object obj) {
                ...
                if (isSucceed && obj != null) {
                    ...
                    InformationHealthListModel model = (InformationHealthListModel) obj;
                    List<InformationHealth> mlist = new ArrayList<>();
                    ...
                    if (CheckUtils.isNoEmptyList(mlist)) {
                        if (isLoadMore) {
                            ...
                            List<InformationHealth> mlistOrg = adapter.getData();
                            if (mlistOrg != null) {
    ...
    public void onClick(View v) {
        switch (v.getId()) {
            case R.id.my_publish_toPay://去支付
                ...
                InformationHealth information = adapter.getData().get(position);
                PayActivity.toPay(mActivity, information.getId(), information.getInformationOrderId(), InformationType.Health.getValue(), information.getAgentId());
                break;
            case R.id.my_publish_republic://重新发布
                ...
                getInformationFeeList(adapter.getData().get(mPos).getId(), adapter.getData().get(mPos).getAgentId(),
                        InformationType.Health.getValue(), adapter.getData().get(mPos).getCategoryId(), OrderType.SendInformation.getValue());
                break;
...
//                hidePopup();
            ...
            case R.id.tv_sticky://置顶
                ...
                getInformationFeeList(adapter.getData().get(mPos).getId(), adapter.getData().get(mPos).getAgentId(),
                        InformationType.Health.getValue(), adapter.getData().get(mPos).getCategoryId(), OrderType.TopInformation.getValue());
                break;
    ...
    protected void showList(final ArrayList<InformationFreeStandard> value, final int orderType) {
        if (CheckUtils.isEmptyList(value)) {
            if (orderType == OrderType.RefreshInformation.getValue()) {
                ...
                createFreeInformationOrder(adapter.getData().get(mPos), InformationType.Health.getValue(), orderType);
    ...
    private void republic(final InformationFreeStandard informationFreeStandard, InformationHealth information) {
        ...
        map.put("hospital", information.getHospital());
        map.put("departments", information.getDepartments());
        map.put("professionalTitle", information.getProfessionalTitle());
        map.put("doctorExpertise", information.getDoctorExpertise());
