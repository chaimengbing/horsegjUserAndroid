...
import com.project.mgjandroid.ui.activity.repair.MyRepairPublishListAdapter;
 ...
 */
public class MyPublishHomeMakingFragment extends BaseFragment implements PullToRefreshBase.OnRefreshListener2<ListView>, View.OnClickListener {
    ...
    private void initData() {
        ...
        adapter = new MyHomemakingPublishListAdapter(R.layout.item_my_publish_education_list_view, mActivity);
    ...
    private void initView() {
        ...
        dialog = new CallPhoneDialog(mActivity, new CallPhoneDialog.onBtnClickListener() {
            @Override
            public void onSure() {
                HomemakingInformation information = adapter.getData().get(mPos);
                dialog.dismiss();
    ...
    private void doDelete(HomemakingInformation information) {
        mMLoadingDialog.show(mActivity.getFragmentManager(), "");
    ...
    public void findAgent(int position) {
        mMLoadingDialog.show(mActivity.getFragmentManager(), "");
        HomemakingInformation bean = adapter.getData().get(position);
        Map<String, Object> map = new HashMap<>();
    ...
    public void onClick(View v) {
        switch (v.getId()) {
            case R.id.my_publish_toPay://去支付
                ...
                HomemakingInformation information = adapter.getData().get(position);
                PayActivity.toPay(mActivity, information.getId(), information.getInformationOrderId(), InformationType.Homemaking.getValue(), information.getAgentId());
                break;
            case R.id.my_publish_republic://重新发布
                ...
                getInformationFeeList(adapter.getData().get(mPos).getId(), adapter.getData().get(mPos).getAgentId(),
                        InformationType.Homemaking.getValue(), adapter.getData().get(mPos).getCategoryId(), OrderType.SendInformation.getValue());
                break;
...
//                hidePopup();
            ...
            case R.id.tv_sticky://置顶
                ...
                getInformationFeeList(adapter.getData().get(mPos).getId(), adapter.getData().get(mPos).getAgentId(),
                        InformationType.Homemaking.getValue(), adapter.getData().get(mPos).getCategoryId(), OrderType.TopInformation.getValue());
                break;
    ...
    protected void showList(final ArrayList<InformationFreeStandard> value, final int orderType) {
        if (CheckUtils.isEmptyList(value)) {
            if (orderType == OrderType.RefreshInformation.getValue()) {
                ...
                createFreeInformationOrder(adapter.getData().get(mPos), InformationType.Homemaking.getValue(), orderType);
    ...
    private void republic(final InformationFreeStandard informationFreeStandard, HomemakingInformation information) {
