...
import com.project.mgjandroid.model.HomeMakingModel;
...
@Router("housekeeping")
public class HomeMakingActivity extends BaseActivity implements PullToRefreshBase.OnRefreshListener2, AdapterView.OnItemClickListener {
    public static final String IS_FROM_HOME_MAKING_ACTIVITY = "home_making_activity";
    ...
    @InjectView(R.id.home_making_list_view)
    private PullToRefreshListView mListView;
    @InjectView(R.id.home_making_container)
    ...
    //1.收购 2.出售
//    private int type;
    private HomeMakingListAdapter mAdapter;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_home_making);
...
//        type = ChooseCityModel.getInstance().getSecondHandType();
        ...
        mAdapter = new HomeMakingListAdapter(R.layout.item_home_making_list_view, mActivity);
        ...
        emptyView = mInflater.inflate(R.layout.empty_view_publish_new, null);
...
//        mListView.setEmptyView(emptyView);
    ...
    private void getBanner() {
        ...
        map.put("informationType", InformationType.Housekeeping.getValue());
        ...
        switch (v.getId()) {
            ...
            case R.id.tv_to_publish:
                if (CommonUtils.checkLogin(mActivity)) {
                    Intent publish = new Intent(mActivity, PublishHomeMakingInfoActivity.class);
    ...
    public void getTag() {
        ...
        operater.doRequest(Constants.URL_HOME_MAKING_HOT_CATEGORY_LIST, null, new VolleyOperater.ResponseListener() {
    ...
    private void getData() {
        ...
        if (categoryId != -1) map.put("homemakingCategoryId", categoryId);
        if (ChooseCityModel.getInstance().getCityCode() != 0)
            map.put("cityCode", ChooseCityModel.getInstance().getCityCode());
        if (ChooseCityModel.getInstance().getProvinceId() != 0)
            map.put("province", ChooseCityModel.getInstance().getProvinceId());
        ...
        if (ChooseCityModel.getInstance().getDistrictId() != 0)
            map.put("district", ChooseCityModel.getInstance().getDistrictId());
        VolleyOperater<HomeMakingModel> operater = new VolleyOperater<>(mActivity);
        operater.doRequest(Constants.URL_HOME_MAKING_LIST, map, new VolleyOperater.ResponseListener() {
            @Override
            public void onRsp(boolean isSucceed, Object obj) {
...
//                            adapter.notifyDataSetChanged();
                        ...
                        if (START != 0) {
                            ...
                            mAdapter.setData(mlist);
                            mListView.getRefreshableView().removeFooterView(emptyView);
                            mListView.getRefreshableView().addFooterView(emptyView);
                        }
                    }
                }
            }
        }, HomeMakingModel.class);
    }
...
//                        ChooseCityModel.getInstance().setType(type);
                ...
                textView.setOnClickListener(new View.OnClickListener() {
                    @Override
                    public void onClick(View v) {
                        Intent allTag = new Intent(mActivity, HomemakingCategoryActivity.class);
                        allTag.putExtra("from", HomeMakingActivity.IS_FROM_HOME_MAKING_ACTIVITY);
