 ...
 */
public class MyPublishEducationFragment extends BaseFragment implements PullToRefreshBase.OnRefreshListener2<ListView>, View.OnClickListener {
        ...
        listView.setOnItemClickListener(new AdapterView.OnItemClickListener() {
            ...
            public void onItemClick(AdapterView<?> parent, View view, int position, long id) {
                if (position >= listView.getRefreshableView().getHeaderViewsCount()) {
                    ...
                    InformationDetailActivity.toInformationDetail(mActivity, information.getId(), InformationType.Education.getValue());
                }
            }
        });
    ...
    public void onClick(View v) {
        switch (v.getId()) {
            case R.id.my_publish_toPay://去支付
                ...
                PayActivity.toPay(mActivity, information.getId(), information.getInformationOrderId(), InformationType.Education.getValue(), information.getAgentId());
                break;
            case R.id.my_publish_republic://重新发布
                ...
                getInformationFeeList(adapter.getData().get(mPos).getId(), adapter.getData().get(mPos).getAgentId(),
                        InformationType.Education.getValue(), adapter.getData().get(mPos).getCategoryId(), OrderType.SendInformation.getValue());
                break;
...
//                hidePopup();
            ...
            case R.id.tv_sticky://置顶
                ...
                getInformationFeeList(adapter.getData().get(mPos).getId(), adapter.getData().get(mPos).getAgentId(),
                        InformationType.Education.getValue(), adapter.getData().get(mPos).getCategoryId(), OrderType.TopInformation.getValue());
                break;
    ...
    protected void showList(final ArrayList<InformationFreeStandard> value, final int orderType) {
        if (CheckUtils.isEmptyList(value)) {
            if (orderType == OrderType.RefreshInformation.getValue()) {
                ...
                createFreeInformationOrder(adapter.getData().get(mPos), InformationType.Education.getValue(), orderType);
    ...
    private void republic(final InformationFreeStandard informationFreeStandard, EducationInformation information) {
        ...
        map.put("title", information.getOrganizationName());
        ...
        map.put("organizationName", information.getOrganizationName());
                    ...
                    if (model.getCode() == 0) {
                        if (BigDecimal.ZERO.compareTo(informationFreeStandard.getPrice()) < 0) {
                            PayActivity.toPay(mActivity, model.getValue().getId(), model.getValue().getInformationOrder().getId(),
                                    InformationType.Education.getValue(), model.getValue().getAgentId());
                        } else {
                            getData(false);
                        }
                    }
