 ...
 */
public class MyPublishRecruitFragment extends BaseFragment implements PullToRefreshBase.OnRefreshListener2<ListView>, View.OnClickListener {

    @InjectView(R.id.list_view)
    private PullToRefreshListView listView;
    private MyPublishRecruitListAdapter adapter;
    private int start = 0;
    ...
    private void initData() {
        ...
        adapter = new MyPublishRecruitListAdapter(R.layout.item_my_publish_new_recruit, mActivity);
        ...
        listView.setOnItemClickListener(new AdapterView.OnItemClickListener() {
            ...
            public void onItemClick(AdapterView<?> parent, View view, int position, long id) {
                if (position >= listView.getRefreshableView().getHeaderViewsCount()) {
                    RecruitInformation information = adapter.getItem(position - listView.getRefreshableView().getHeaderViewsCount());
                    InformationDetailActivity.toInformationDetail(mActivity, information.getId(), InformationType.Recruit.getValue());
                }
            }
        });
    ...
    private void initView() {
        ...
        dialog = new CallPhoneDialog(mActivity, new CallPhoneDialog.onBtnClickListener() {
            @Override
            public void onSure() {
                RecruitInformation information = adapter.getData().get(mPos);
                dialog.dismiss();
    ...
    private void doDelete(RecruitInformation information) {
        mMLoadingDialog.show(mActivity.getFragmentManager(), "");
        Map<String, Object> map = new HashMap<>();
        map.put("informationType", InformationType.Recruit.getValue());
        map.put("informationId", information.getId());
        VolleyOperater<InformationPositionListModel> operater = new VolleyOperater<>(mActivity);
        ...
        }, InformationPositionListModel.class);
    }
    ...
    public void getData(final boolean isLoadMore) {
        ...
        map.put("informationType", InformationType.Recruit.getValue());
        map.put("viewType", 1);
        VolleyOperater<InformationRecruitListModel> operater = new VolleyOperater<>(mActivity);
        operater.doRequest(Constants.URL_FIND_INFORMATION_LIST, map, new VolleyOperater.ResponseListener() {
            ...
            public void onRsp(boolean isSucceed, Object obj) {
                ...
                if (isSucceed && obj != null) {
                    ...
                    InformationRecruitListModel model = (InformationRecruitListModel) obj;
                    List<RecruitInformation> mlist = new ArrayList<>();
                    ...
                    if (CheckUtils.isNoEmptyList(mlist)) {
                        if (isLoadMore) {
                            ...
                            List<RecruitInformation> mlistOrg = adapter.getData();
                            if (mlistOrg != null) {
        ...
        }, InformationRecruitListModel.class);
    }
    ...
    public void onClick(View v) {
        switch (v.getId()) {
            ...
            case R.id.my_publish_toPay://去支付
                ...
                RecruitInformation information = adapter.getData().get(position);
                PayActivity.toPay(mActivity, information.getId(), information.getInformationOrderId(), InformationType.Recruit.getValue(), information.getAgentId());
                break;
            case R.id.my_publish_republic://重新发布
                ...
                getInformationFeeList(adapter.getData().get(mPos).getId(), adapter.getData().get(mPos).getAgentId(),
                        InformationType.Recruit.getValue(), adapter.getData().get(mPos).getCategoryId(), OrderType.SendInformation.getValue());
                break;
...
//                hidePopup();
            ...
            case R.id.tv_sticky://置顶
                ...
                getInformationFeeList(adapter.getData().get(mPos).getId(), adapter.getData().get(mPos).getAgentId(),
                        InformationType.Recruit.getValue(), adapter.getData().get(mPos).getCategoryId(), OrderType.TopInformation.getValue());
                break;
            ...
            case R.id.iv_more_setting:
                ...
                if (adapter.getData().get(mPos).getIsExpire() == 0 && adapter.getData().get(mPos).getStatus() == 2) {
                    ...
                    if (mPos == adapter.getData().size() - 1 && adapter.getData().size() != 1) {
                        mPopupWindow.showAsDropDown(v, -(int) mActivity.getResources().getDimension(R.dimen.x120), -(int) mActivity.getResources().getDimension(R.dimen.x126));
                    } else {
                        mPopupWindow.showAsDropDown(v, -(int) mActivity.getResources().getDimension(R.dimen.x120), -(int) mActivity.getResources().getDimension(R.dimen.x20));
                    }
                } else {
                    if (dialog != null && !dialog.isShowing())
                        dialog.show();
    ...
    protected void showList(final ArrayList<InformationFreeStandard> value, final int orderType) {
        if (CheckUtils.isEmptyList(value)) {
            if (orderType == OrderType.RefreshInformation.getValue()) {
                ...
                createFreeInformationOrder(adapter.getData().get(mPos), InformationType.Recruit.getValue(), orderType);
    ...
    private void republic(final InformationFreeStandard informationFreeStandard, RecruitInformation information) {
        ...
        map.put("companyAddress", information.getCompanyAddress());
        map.put("companyType", information.getCompanyType());
        map.put("profession", information.getProfession());
        map.put("companyScale", information.getCompanyScale());
        map.put("positionName", information.getPositionName());

        map.put("recruitNum", information.getRecruitNum());
        map.put("education", information.getEducation());
        map.put("workYears", information.getWorkYears());
        map.put("claim", information.getClaim());
        map.put("salary", information.getSalary());
        map.put("welfare", information.getWelfare());
