 ...
 */
public class MyRentHouseListAdapter extends BaseListAdapter<HouseLeaseInformation> implements View.OnClickListener {
    private PopupWindow mPopupWindow;
    private int mPos;
    private View.OnClickListener mListener;

    public MyRentHouseListAdapter(int layoutId, Activity mActivity, View.OnClickListener listener) {
    ...
    protected void getRealView(ViewHolder holder, HouseLeaseInformation bean, int position, View convertView, ViewGroup parent) {
        ...
        if (showAmt.contains("-")) {
            String[] split = showAmt.split("-");
            tvPrice.setText(split[0] + "-" + split[1]);
        } else {
