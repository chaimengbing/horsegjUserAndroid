...
public class HeaderViewPagerLayout extends LinearLayout {
    ...
    public HeaderViewPagerLayout(Context context, AttributeSet attrs) {
        ...
        TypedArray a = context.obtainStyledAttributes(attrs,
                R.styleable.HeaderViewPagerLayout);

        mTopHeight = a.getDimensionPixelSize(
                R.styleable.HeaderViewPagerLayout_topHeight, (int) getResources().getDimension(R.dimen.title_bar_height));// 默认为0dp
        isScroller = a.getBoolean(R.styleable.HeaderViewPagerLayout_isScroller, false);
    ...
    protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
        ...
        maxY = mHeadView.getMeasuredHeight() - mTopHeight;
    ...
    public void setIsScroller(boolean isScroller) {
        if (this.isScroller != isScroller) {
...
//                MLog.e("ACTION_DOWN");
            ...
            case MotionEvent.ACTION_MOVE:
                ...
                mLastY = currentY;
                if (actionDownFlag && shiftX > mTouchSlop && shiftX > shiftY) {
                    //水平滑动
                    verticalScrollFlag = false;
                    actionDownFlag = false;
                } else if (actionDownFlag && shiftY > mTouchSlop && shiftY > shiftX) {
                    //垂直滑动
                    verticalScrollFlag = true;
                    actionDownFlag = false;
                }
//                MLog.e("ACTION_MOVE:" + verticalScrollFlag);
                /**
                ...
                if (verticalScrollFlag && (shiftX > mTouchSlop || shiftY > mTouchSlop)) {
                    int action = ev.getAction();
                    ev.setLocation(mDownX, currentY);
