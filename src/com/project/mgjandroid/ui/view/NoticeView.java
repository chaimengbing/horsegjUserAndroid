...
/**
 * Created by yuandi on 2016/9/23.
 */
public class NoticeView extends FrameLayout implements View.OnClickListener {
     ...
     * @param list
     */
    public void setBroadList(List<PromotionActivity> list, Context context, boolean topMargin) {
        // 设置集合的时候，要将上一次的集合清除。
        if (list == null || list.size() == 0) {
            return;
        }
        ...
        // 创建View
        for (int i = 0; i < list.size(); i++) {
            PromotionActivity promotion = list.get(i);
...
//            params.topMargin = getResources().getDimensionPixelOffset(R.dimen.x2);
            ...
            if (topMargin)
                paramsChild.topMargin = getResources().getDimensionPixelOffset(R.dimen.x15);
            mNoticeList.add(childLayout);
            childLayout.setVisibility(INVISIBLE);
            addView(childLayout);
        }
        mCurrentNotice = 0;
        mNoticeList.get(mCurrentNotice).setVisibility(VISIBLE);
     ...
     * @param list
     */
    public void setNoticeList(List<Broadcast> list) {
        // 设置集合的时候，要将上一次的集合清除。
        if (list == null || list.size() == 0) {
            return;
        }
        ...
        // 创建TextView
        for (int i = 0; i < list.size(); i++) {
            TextView textView = createTextView(list.get(i).getTitle());
            textView.setTag(list.get(i));
            mNoticeList.add(textView);
            addView(textView);
        }
    ...
    public List<View> getmNoticeList() {
        return mNoticeList;
    }
    ...
    class NoticeRunnable implements Runnable {
        @Override
        public void run() {
            if (mNoticeList == null || mNoticeList.size() <= mCurrentNotice) {
                mHandler.removeCallbacks(mNoticeRunnalbe);
                return;
            }
            // 隐藏当前的textView
            View currentView = mNoticeList.get(mCurrentNotice);
            ...
            // 显示下一个TextView
            View nextView = mNoticeList.get(mCurrentNotice);
