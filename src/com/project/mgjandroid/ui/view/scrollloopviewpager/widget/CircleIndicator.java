...
public class CircleIndicator extends View implements ViewPager.OnPageChangeListener, LoopViewPager.OnPageChangeListener {
    ...
    private int unselected_color = 0x80ffffff;
    ...
    private void handlerAttrs(Context context, AttributeSet attrs) {
        ...
        dotWidth = ta.getDimensionPixelOffset(R.styleable.CircleIndicator_radius_my, dip2px(context, dotWidth));
        dotHeight = ta.getDimensionPixelOffset(R.styleable.CircleIndicator_radius_my, dip2px(context, dotHeight));
    ...
    protected void onDraw(Canvas canvas) {
        ...
        for (int i = 0; i < mCount; i++) {
            ...
            r2.right = x + dotWidth;
            r2.top = y;
            r2.bottom = y + dotHeight;
            canvas.drawRoundRect(r2, radius, radius, selected ? selectedPaint : unselectedPaint);
            x += (dotWidth + dotMargin);
        }
    }

    public void setDotSize(int dotWidth, int dotHeight, int paddingBottom, int dotMargin, int radius) {
        this.dotWidth = dip2px(getContext(), dotWidth);
        this.dotHeight = dip2px(getContext(), dotHeight);
        this.paddingBottom = dip2px(getContext(), paddingBottom);
        this.dotMargin = dip2px(getContext(), dotMargin);
        this.radius = dip2px(getContext(), radius);
        invalidate();
    ...
    protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
        ...
        int mViewHeight = getMeasurement(heightMeasureSpec, dotHeight
    ...
    public void setViewPager(ViewGroup viewpager) {
        ...
        actualWidth = dotMargin * (mCount - 1) + dotWidth * mCount;
        invalidate();
    }
