...
public class VolleyOperater<T> {
    ...
    public String setBody(Map<String, Object> mapParams) {
        ...
        if (userInfo != null) {
            ...
            String token = PreferenceUtils.getStringPreference("token", "", App.getInstance());
            if (!"".equals(token)) {
                map.put("token", token);
            }
        }
        map.put("app", App.getInstance().getString(R.string.app_nick));
...
//		map.put("apiVersion", "1.0.0");
        ...
        map.put("clientId", JPushInterface.getRegistrationID(App.getContext()));
//		map.put("brand", DeviceParameter.getDeviceProduct());
        ...
        String localMacAddress = DeviceParameter.getAdresseMAC(App.getInstance());
...
//        }
    ...
    public String getChannel() {
        ...
        try {
            ApplicationInfo appInfo = App.getInstance().getPackageManager().getApplicationInfo(context.getPackageName(), PackageManager.GET_META_DATA);
            ...
            try {
                if (!JSONObject.NULL.equals(response)) {
                    if (response instanceof ErrorModel) {
                        if (((ErrorModel) response).getCode() == 100004) {
                            listener.onRsp(true, "100004");
                        } else {
