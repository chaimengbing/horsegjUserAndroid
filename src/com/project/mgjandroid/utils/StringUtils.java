...
public class StringUtils {
     ...
     * @param context             上下文
     * @param resId               字符串资源id
     * @param foregroundColorSpan 要设置的字体前景色
     * @return 设置后的字体颜色
     */
    ...
    public static String money2Str(BigDecimal money, String unit) {
        if (money == null)
            return "0" + unit;
        ...
        if (money.compareTo(new BigDecimal("9999.99")) > 0) {
            BigDecimal wanMoney = new BigDecimal(df.format(money.divide(new BigDecimal("10000"))));
            if (wanMoney.compareTo(new BigDecimal(wanMoney.intValue())) == 0) {
                showMoney = wanMoney.intValue() + "万";
            } else {
                showMoney = wanMoney + "万";
            }
        } else {
            if (money.compareTo(new BigDecimal(money.intValue())) == 0) {
                showMoney = money.intValue() + "";
            } else {
                showMoney = money + "";
            }
        }
        return showMoney + unit;
    }
    ...
    public static SpannableString getPrice(Context context, String amt) {
        String yuan = "元", wanyuan = "万元", yuanyue = "元/月", wanyuanyue = "万元/月";
        SpannableString spannableString = new SpannableString(amt);
        while (true) {
            int position = amt.lastIndexOf(wanyuanyue);
            if (position == -1)
                break;
            spannableString = setTextSize(spannableString, position, position + wanyuanyue.length(), DipToPx.sp2px(context, 14));
            amt = amt.substring(0, position);
        }
        while (true) {
            int position = amt.lastIndexOf(yuanyue);
            if (position == -1)
                break;
            spannableString = setTextSize(spannableString, position, position + yuanyue.length(), DipToPx.sp2px(context, 14));
            amt = amt.substring(0, position);
        }
        while (true) {
            int position = amt.lastIndexOf(wanyuan);
            if (position == -1)
                break;
            spannableString = setTextSize(spannableString, position, position + wanyuan.length(), DipToPx.sp2px(context, 14));
            amt = amt.substring(0, position);
        }
        while (true) {
            int position = amt.lastIndexOf(yuan);
            if (position == -1)
                break;
            spannableString = setTextSize(spannableString, position, position + yuan.length(), DipToPx.sp2px(context, 14));
            amt = amt.substring(0, position);
        }
        return spannableString;
    }

    public static SpannableString setTextSize(SpannableString spannableString, int startIndex, int endIndex, int fontSize) {
        spannableString.setSpan(new AbsoluteSizeSpan(fontSize), startIndex, endIndex, Spanned.SPAN_EXCLUSIVE_EXCLUSIVE);
        spannableString.setSpan(new ForegroundColorSpan(Color.parseColor("#666666")), startIndex, endIndex, Spanned.SPAN_EXCLUSIVE_EXCLUSIVE);
        return spannableString;
    }

    public static String stringInsertString(String content, String[] strs) {
