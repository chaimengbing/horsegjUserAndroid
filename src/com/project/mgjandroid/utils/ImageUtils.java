...
public class ImageUtils {
     ...
     * 计算缩略图尺寸比例
     *
     * @param options   请求尺寸大小
     * @param reqWidth  实际的大小
     * @param reqHeight 实际的大小
     * @return
     */
    ...
    public static void getBlur(final Context context, String path, final ImageView imageView, String endDes, final View view) {
        if (endDes == null)
            endDes = "";
        ...
        try {
            Glide.with(context).load(path + endDes).asBitmap().into(new SimpleTarget<Bitmap>() {
                @Override
                public void onResourceReady(Bitmap resource, GlideAnimation<? super Bitmap> glideAnimation) {
                    if (null != resource)
                        blur(context, resource, imageView, view, true);
                    else {
                        Bitmap bm = BitmapFactory.decodeResource(context.getResources(), R.drawable.default_blur);
                        blur(context, bm.copy(bm.getConfig(), true), imageView, view, false);
                    }
                }
    ...
    public static void blur(Context context, Bitmap bkg, ImageView view, View v, boolean isNet) {
...
//		canvas.drawBitmap(bkg, 0, 0, paint);
    ...
    public static void loadBitmap(Context context, String path, ImageView imageView, int
            defaultDrawable, String endDes) {
        if (endDes == null)
            endDes = "";
        ...
        try {
            Glide.with(context).load(path + endDes).asBitmap().centerCrop
                    ().placeholder(defaultDrawable).into(new MyBitmapImageViewTarget(imageView));
        } catch (OutOfMemoryError e) {
            Glide.get(context).clearMemory();
            try {
                Glide.with(context).load(path + endDes).asBitmap
                        ().centerCrop().placeholder(defaultDrawable).into(new MyBitmapImageViewTarget
                        (imageView));
            } catch (Exception e1) {
    ...
    public static void loadBitmapFit(Context context, String path, ImageView imageView, int defaultDrawable, String endDes) {
        if (endDes == null)
            endDes = "";
        ...
        try {
            Glide.with(context).load(path + endDes).placeholder(defaultDrawable).into(imageView);
        } catch (OutOfMemoryError e) {
            ImageCache.getInstance().clearMemory();
            try {
                Glide.with(context).load(path + endDes).placeholder(defaultDrawable).into(imageView);
    ...
    public static void loadBitmapFitCenter(Context context, String path, ImageView imageView, int defaultDrawable, String endDes) {
        if (endDes == null)
            endDes = "";
        ...
        try {
            Glide.with(context).load(path + endDes).asBitmap().placeholder(defaultDrawable).into(new MyBitmapImageViewTarget
                    (imageView));
        } catch (OutOfMemoryError e) {
            ImageCache.getInstance().clearMemory();
            try {
