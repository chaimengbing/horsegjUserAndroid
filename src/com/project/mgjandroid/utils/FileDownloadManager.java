...
//    private DownloadDBHelper helper;
    private HashMap<String, Object> downloadInfo = new HashMap<>();
    /**
     * 该文件下载的url
     */
    private String url;
...
//        helper = new DownloadDBHelper();
    ...
    public boolean checkFileFinish() {
        long size = PreferenceUtils.getLongPreference("size", 0, App.getContext());
        if (size != 0) {
            mList.clear();
            downloadInfo.put("_startPos", PreferenceUtils.getLongPreference("startPos", 0, App.getContext()));
            downloadInfo.put("_endPos", PreferenceUtils.getLongPreference("endPos", 0, App.getContext()));
            downloadInfo.put("_size", size);
            mList.add(downloadInfo);
        }

        if (isDownloadFinish || isFileDownloadFinish(mList)) {
    ...
    private void startDownload(final ArrayList<HashMap<String, Object>> maps) {
    ...
    private void createDownloadInfos() {
        ...
        downloadInfo.put("_startPos", infos.get(0).startPos);
        downloadInfo.put("_endPos", infos.get(0).endPos);
        downloadInfo.put("_size", infos.get(0).completeSize);
        mList.add(downloadInfo);
    }
     ...
     * 恢复以前的下载信息
     */
    private void revertDownloadInfos(ArrayList<HashMap<String, Object>> maps) {
//        ArrayList<String> columns = DownloadDB.TABLES.DOWNLOAD.getTableColumns();
        for (HashMap<String, Object> map : maps) {
            DownloadInfo info = new DownloadInfo();
//            info.id = Integer.parseInt(map.get(columns.get(0)));
            info.startPos = (Long) map.get("_startPos");
            info.endPos = (Long) map.get("_endPos");
            info.completeSize = (Long) map.get("_size");
     ...
     * 检测该文件是否下载完成
     */
    private boolean isFileDownloadFinish(ArrayList<HashMap<String, Object>> maps) {
        boolean result = true;
        if (maps == null || maps.size() == 0) {
            return false;
        }
//        ArrayList<String> columns = DownloadDB.TABLES.DOWNLOAD.getTableColumns();
        for (HashMap<String, Object> map : maps) {
            //如果完成字节数不足end-start，代表该线程未完成，
            if ((Long) (map.get("_size")) <
                    (Long) (map.get("_endPos")) - (Long) (map.get("_startPos"))) {
                result = false;
                break;
            }
        }
...
//                    helper.updateInfos(url, infos);
                    ...
                    downloadInfo.put("_endPos", infos.get(0).endPos);
                    downloadInfo.put("_size", infos.get(0).completeSize);
                    PreferenceUtils.saveLongPreference("startPos", infos.get(0).startPos, App.getContext());
                    PreferenceUtils.saveLongPreference("endPos", infos.get(0).endPos, App.getContext());
                    PreferenceUtils.saveLongPreference("size", infos.get(0).completeSize, App.getContext());
